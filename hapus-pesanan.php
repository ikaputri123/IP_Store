...
if (!empty($_GET['id'])) {
    ...
    if (mysqli_num_rows($cek_pesanan) > 0) {
        ...
        echo "<script>alert('Pesanan tidak ditemukan atau bukan mikik Anda.'); window.location='cart.php';</script>";
        }
} else {
    echo "<script>alert('Permintaan tidak valid (id kososng).'); window.location='cart.php';</script>";    
}